...
def neud_level(age):
    norm_age = (min(age, 15) - 7) // 2  # Преобразуем возраст в количество циклов вычитания 1.5
...
def ruffier_result(r_index, level):
    ...
    level = level - 4  # Вычитаем 4 для следующего уровня

    if r_index >= level:

        return 1

    level = level - 5  # Вычитаем 5 для следующего уровня

    if r_index >= level:

        return 2

    level = level - 5.5  # Вычитаем 5.5 для следующего уровня

    if r_index >= level:

        return 3

    return 4  # Если не попадает ни в один порог, то возвращаем 4
